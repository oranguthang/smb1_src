...
class LabelRenameManifestTests(unittest.TestCase):
    def test_manifest_roots_match_imported_source_labels(self) -> None:
        ...
        mapped_roots = [item[0] for item in manifest["registries"]["smb1"]["renames"]]
    ...
    def test_manifest_matches_current_source_labels(self) -> None:
        ...
        self.assertEqual(set(manifest["registries"]), {"smb1", "smb2"})
        smb1 = manifest["registries"]["smb1"]
        renames = smb1["renames"]
        additions = smb1["project_additions"]
        ...
        self.assertEqual(
            smb1["addition_columns"],
            ["current", "current_path", "introduced_in_commit", "reason"],
        )
        self.assertEqual(smb1["counts"]["original_labels"], len(renames))
        self.assertEqual(smb1["counts"]["direct_renames"], len(renames))
        self.assertEqual(smb1["counts"]["project_additions"], len(additions))
        ...
        self.assertEqual(smb1["counts"]["current_labels"], len(mapped_labels))
